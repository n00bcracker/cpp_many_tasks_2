...
Mistake LogMistake(const clang::SourceManager& source_manager, const clang::SourceLocation& loc,
                   const std::string& name, const std::string& wrong_word,
...
void NamingRulesViolation::CheckMistakes(const std::string& name,
        ...
        for (auto& word : words) {
            if (word.size() > 3) {
                ...
                if (min_dist > 0 && min_dist < 4) {
                    violations_.mistakes.emplace_back(LogMistake(
                        source_manager, loc, name, std::string(word), std::string(*nearest_word)));
                }
            }
        }
    }
...
void NamingRulesViolation::run(const MatchFinder::MatchResult& result) {
    ...
    if (type_declaration && !source_manager.isInSystemHeader(type_declaration->getLocation())) {
        if (type_declaration->getIdentifier()) {
            ...
            if (!std::regex_match(name, kTypeNamePattern)) {
                violations_.bad_names.emplace_back(
                    LogBadName(source_manager, start_loc, Entity::kType, name));
            } else {
    ...
    const TypeAliasDecl* type_alias_declaration =
        result.Nodes.getNodeAs<TypeAliasDecl>("type_alias_decl");
    if (type_alias_declaration &&
        !source_manager.isInSystemHeader(type_alias_declaration->getLocation())) {
    ...
    if (type_def_declaration &&
        !source_manager.isInSystemHeader(type_def_declaration->getLocation())) {
    ...
    if (method_declaration && !source_manager.isInSystemHeader(method_declaration->getLocation())) {
        if (method_declaration->getIdentifier() && method_declaration->getName() != "run") {
            ...
            if (!std::regex_match(name, kTypeNamePattern)) {
                violations_.bad_names.emplace_back(
                    LogBadName(source_manager, start_loc, Entity::kFunction, name));
            } else {
                CheckMistakes(name, source_manager, start_loc);
            }
        }
        ...
        for (const auto* param_declaration : method_declaration->parameters()) {
            if (param_declaration->getIdentifier()) {
                ...
                if (param_declaration->getType().isConstQualified() ||
                    param_declaration->isConstexpr()) {
                    if (!std::regex_match(name, kConstPattern)) {
                        violations_.bad_names.emplace_back(
                            LogBadName(source_manager, start_loc, Entity::kConst, name));
                    } else {
                        CheckMistakes(name, source_manager, start_loc);
                    }
                } else {
                    if (!std::regex_match(name, kPublicFieldPattern)) {
                        violations_.bad_names.emplace_back(
                            LogBadName(source_manager, start_loc, Entity::kVariable, name));
                    } else {
                        CheckMistakes(name, source_manager, start_loc);
                    }
                }
    ...
    const CXXConstructorDecl* constructor_declaration =
        result.Nodes.getNodeAs<CXXConstructorDecl>("construct_decl");
    if (constructor_declaration &&
        !source_manager.isInSystemHeader(constructor_declaration->getLocation())) {
    ...
    const CXXDestructorDecl* destructor_declaration =
        result.Nodes.getNodeAs<CXXDestructorDecl>("destruct_decl");
    if (destructor_declaration &&
        !source_manager.isInSystemHeader(destructor_declaration->getLocation())) {
            ...
            if (field_declaration->getType().isConstQualified()) {
                ...
                if (field_declaration->getAccess() == AccessSpecifier::AS_private) {
                    if (!std::regex_match(name, kPrivateFieldPattern)) {
                        violations_.bad_names.emplace_back(
                            LogBadName(source_manager, start_loc, Entity::kField, name));
                    } else {
                        CheckMistakes(name, source_manager, start_loc);
                    }
