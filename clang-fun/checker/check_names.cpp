...
static llvm::cl::opt<std::string> spell_checker_dict_file(
    "dict", llvm::cl::desc("Specify filename with spell checker dictionary"),
    ...
    for (auto& file_path : options_parser.getSourcePathList()) {
        ...
        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     cxxRecordDecl(unless(isImplicit()), unless(classTemplateSpecializationDecl()),
        ...
        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     typeAliasDecl(unless(isImplicit()), unless(isExpansionInSystemHeader()))
                         .bind("type_alias_decl")),
            &naming_rules_violations);

        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     typedefDecl(unless(isImplicit()), unless(isExpansionInSystemHeader()))
                         .bind("type_def_decl")),
            &naming_rules_violations);

        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     cxxMethodDecl(unless(isImplicit()), unless(cxxConstructorDecl()),
                                   unless(cxxDestructorDecl()), unless(isExpansionInSystemHeader()))
                         .bind("method_decl")),
            &naming_rules_violations);

        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     cxxConstructorDecl(unless(isImplicit()), unless(isExpansionInSystemHeader()))
                         .bind("construct_decl")),
            &naming_rules_violations);

        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     cxxDestructorDecl(unless(isImplicit()), unless(isExpansionInSystemHeader()))
                         .bind("destruct_decl")),
            &naming_rules_violations);
        ...
        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     varDecl(unless(parmVarDecl()), unless(isExpansionInSystemHeader()))
                         .bind("variable_decl")),
            &naming_rules_violations);

        finder.addMatcher(
            traverse(clang::TK_IgnoreUnlessSpelledInSource,
                     functionDecl(unless(cxxMethodDecl()), unless(isExpansionInSystemHeader()))
                         .bind("func_decl")),
            &naming_rules_violations);

        finder.addMatcher(enumDecl(unless(isExpansionInSystemHeader())).bind("enum_decl"),
                          &naming_rules_violations);
