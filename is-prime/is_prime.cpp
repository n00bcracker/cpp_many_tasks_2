...
bool IsPrime(uint64_t x) {
    ...
    is_not_prime.clear();
    if ((bound - 1) / kThreadCnt < 10) {
        ThreadFunc(x, 2, bound);
    } else {
        std::vector<std::thread> threads;
        uint64_t l_bound = 2;
        uint64_t std_interval_len = (bound - 1) / kThreadCnt;
        uint64_t full_threads_cnt = (bound - 1) % kThreadCnt;
        for (size_t i = 0; i < kThreadCnt; ++i) {
            auto interval_len = i < full_threads_cnt ? std_interval_len + 1 : std_interval_len;
            threads.emplace_back(ThreadFunc, x, l_bound, l_bound + interval_len - 1);
