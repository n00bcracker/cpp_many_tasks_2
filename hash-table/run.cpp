...
TEST_CASE("Benchmark") {
    ...
    ConcurrentHashMap<int, int> map(kNumIterations, kNumThreads);
    ...
    map.Clear();
    BENCHMARK("SpecialInsertions:" + std::to_string(kNumThreads)) {
        Runner runner{kNumIterations};
        EqualLowBits elb{16};
        runner.Do([&map, elb]() mutable { map.Insert(elb(), 1); });
        for (auto i : std::views::iota(1, static_cast<int>(kNumThreads))) {
    ...
    map.Clear();
    BENCHMARK("ManySearches:" + std::to_string(kNumThreads)) {
        Runner runner{kNumIterations};
        runner.Do([&map, rand = Random{kSeed - 1}]() mutable { map.Insert(rand(), 1); });
        for (auto i : std::views::iota(1u, kNumThreads)) {
    ...
    map.Clear();
    BENCHMARK("Deletions:" + std::to_string(kNumThreads)) {
        Runner runner{kNumIterations};
        for (auto i : std::views::iota(0u, kNumThreads)) {
